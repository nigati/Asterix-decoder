...
<script lang="ts" type="module">
  import { FileEarmarkPlus, House, Table, Plus } from "svelte-bootstrap-icons";
  ...
  async function htmlToCsvAll(filename: string) {
  var tableRows = document.querySelectorAll("table tr");

  for (var i = 0; i < tableRows.length; i++) {
    var row = [];
    var cols = tableRows[i].querySelectorAll("td, th");
    ...
    for (var j = 0; j < cols.length; j++) {
      ...
      if (cols[j].classList.contains("expandedColumn")) {
        ...
        if (expandButton) {
          ...
          if (expandedData) {
            cellData = Array.from(expandedData)
  ...
  var nextPageButton = document.querySelector("button.btn-next") as HTMLButtonElement;

  if (nextPageButton && !nextPageButton.disabled) {
    nextPageButton.dispatchEvent(new MouseEvent("click")); // Dispatch a click event on the next page button
    setTimeout(htmlToCsvAll.bind(null, filename), 1); // Wait for the next page to load and export it
  } 
  else {
    downloadCSVFile(csv.join("\n"), filename); // Download the CSV file when all pages have been processed
  }
}

  async function htmlToCsv(filename: any) {
    var csv: string[] = [];
  ...
  async function expandAllItems1() {
    expandedRows = items;
    await htmlToCsv("file");
    csv=[];
  }
  async function expandAllItems2() {
    expandedRows = items;
    await htmlToCsvAll("file");
    csv=[];
    setPage(0);
  }
</script>
  ...
  <nav id="sidebar" class="{activeWindow === 'home' ? 'active' : ''}">
    ...
    <ul class="list-unstyled components mb-5">
      <li class="{activeWindow === 'home' ? 'active' : ''}">
        <a href="#home" on:click="{() => handleNavClick('home')}">
          <House /> Home
        </a>
      </li>
      <li class="{activeWindow === 'load' ? 'active' : ''}">
        <a href="#load" on:click="{() => handleNavClick('load')}">
          <FileEarmarkPlus /> Load File
        </a>
      </li>
      <li class="{activeWindow === 'table' ? 'active' : ''} {items.length === 0 ? 'disabled' : ''}">
        <a href="#table" on:click="{() => handleNavClick('table')}">
          <Table /> Table
        </a>
      </li>
      ...
      <li class="{activeWindow === 'extra' ? 'active' : ''}">
        <a href="#extra" on:click="{() => handleNavClick('extra')}">
          <Plus /> Extra
        </a>
      </li>
    </ul>
